...
destinations.get(
  ...
  (request, response) => {
    ...
    const query = `SELECT work_items.work_order_id, work_items.depature_time, work_items.id as workitemid, work_orders.id as workorderid, work_orders.name from work_items INNER JOIN destinations ON (work_items.destination_id = destinations.id) INNER JOIN work_orders ON (work_items.work_order_id = work_orders.id) WHERE destinations.id = ${id}`;
    db.manyOrNone(query)
      .then(destinationRes => {
